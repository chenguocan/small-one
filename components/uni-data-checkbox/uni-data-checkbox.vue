	...
	<view class="uni-data-checklist">
		...
		<template v-else>
			<checkbox-group v-if="multiple" class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}" @change="chagne">
				<label class="checklist-box" style="margin-left: 20rpx;" :class="item.labelClass" :style="[item.styleBackgroud]" v-for="(item,index) in dataList"
				 :key="index">
					...
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<view>
							<view class="checklist-text" :class="item.textClass" style="font-weight: 700;" :style="[item.styleIconText]">{{item.text}}</view>
							<view style="font-weight: normal;font-size: 24rpx;">有效期至{{item.start}}</view>
						</view>
						<view style="margin: 0 20rpx;">
							<view style="color: red;font-weight: 700;" class="checklist-text" :class="item.textClass" >￥{{item.amount}}</view>
							<view style="font-weight: normal;font-size: 24rpx;">满{{item.minimum}}可用</view>
						</view>
						<image style="margin-left:10rpx;width: 120rpx;height: 120rpx;" :src="item.image"></image>
