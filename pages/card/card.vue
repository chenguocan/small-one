		...
		<view class="goods-list">
			...
			<view class="row" v-for="(item,index) in goodsList" :key="index">
				...
				<view class="menu" @tap="handleSingleDelete(item)">
					<view class="icon iconfont icondelete-o"></view>
				</view>
				<!-- 商品 -->
				<view class="production" @touchstart="handleTouchStart(index,$event)"
					@touchmove="handleTouchMove(index,$event)" @touchend="handleTouchEnd(index,$event)"
					...
					<view class="goods-info" @tap="handleGoodsInfo(item)">
						...
						<view class="info">
							...
							<view class="spec">{{item.sub_title}}</view>
							<view class="price-number">
								<view class="price">￥{{(item.discount_price/100).toFixed(2)}}</view>
	...
	export default {
		...
		methods: {
			...
			sum() { // 合计
				...
				this.goodsList.forEach((item, i) => {
					if (item.selected) {
						this.sumPrice = this.sumPrice + (parseFloat(item.number) * parseFloat(item.discount_price /
							100).toFixed(2))
					}
				})
