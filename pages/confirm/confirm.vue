			...
			<view class="row" v-for="(item,index) in goodsList" :key="index">
				<view class="goods-info">
					...
					<view class="info">
						...
						<view class="spec">选择: {{item.title}} 数量: {{item.number}}</view>
						<view class="price-number">
							<view class="price">￥{{item.discount_price * item.number}}</view>
						</view>
					</view>
				</view>
			</view>
			...
			uni.getStorage({
				...
				success: res => {
					...
					for (let i = 0; i < this.goodsList.length; i++) {
						this.goodsPrice = this.goodsPrice + (this.goodsList[i].number * this.goodsList[i].discount_price);
					}
