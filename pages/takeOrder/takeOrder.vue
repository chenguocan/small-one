<template>
	...
	<view class="container">
		...
		<view class="footer" v-if="plateActive==0">
			<image @click="toShowCard(totalGoodsNum)" src="/static/image/food/buy.png"></image>

			<view class="price-count" v-if="totalGoodsNum==0">未选购商品</view>
			...
			<view class="btn">
				...
				<view class="submit-order" @click="onBuy(totalGoodsNum)">
					去结算
				</view>
			</view>
			<!-- <view class="buy">
...
<script>
	...
	export default {
		...
		onLoad: async function() {
			...
			if (register === true) {} else if (register === false) {
				this.toLogin()
			}
			this.getBanner();
		},
		...
		onShow: async function() {
			await this.$onLaunched
			let register = uni.getStorageSync('register');
			let time = uni.getStorageSync('takeout_food_time');
			let statustime = dayjs().add(2, 'hour').format('YYYY-MM-DD HH:mm')
			let currentTime = dayjs().format('YYYY-MM-DD HH:mm');

			if (!time || time < currentTime) {
				uni.removeStorageSync('takeout_goodsInfo')
				uni.removeStorageSync('takeout_jtjd_order');
				uni.removeStorageSync('takeout_sort_new');
				uni.setStorageSync('takeout_food_time', statustime);
			}
			if (!this.refresh) {
			...
			if (!uni.getStorageSync('takeout_jtjd_order') || uni.getStorageSync('takeout_jtjd_order').length == 0) {
				let sort = this.sort;
		...
		methods: {
					...
					uni.showModal({
						...
						success(res) {
							if (res.cancel) {} else {
								...
								uni.setStorageSync('takeout_goodsInfo', goodsInfo);
								...
								if (uni.getStorageSync('takeout_jtjd_order')) {
									let order = uni.getStorageSync('takeout_jtjd_order');

									if (order.length == 0) {
										order.push(list);
										uni.setStorageSync('takeout_jtjd_order', order);
									} else {
										let flag = true;
			...
			getItemGroupAboutClient() {
				...
				if (uni.getStorageSync('takeout_jtjd_order') && uni.getStorageSync('takeout_sort_new') && uni
					.getStorageSync('takeout_jtjd_order')
					.length >
					0) {
					let sort = uni.getStorageSync('takeout_sort_new');
					let goodsList = [];
					console.log(sort);
					that.refresh = true;
					for (let i = 0; i < sort.length; i++) {
						...
						if (i == sort.length - 1) {
							...
							if (uni.getStorageSync('takeout_goodsInfo')) {
								let goodsInfo = uni.getStorageSync('takeout_goodsInfo'); // console.log(goodsInfo);
					...
					uni.request({
						...
						success: function(res) {
							...
							if (res.data.errCode == 0) {
								...
								uni.setStorageSync('takeout_sort_new', sort);
								let goodsList = [];
								that.sort = sort;
								console.log(sort);
								for (let i = 0; i < sort.length; i++) {
									...
									if (i == (sort.length) - 1) {
										...
										if (uni.getStorageSync('takeout_goodsInfo')) {
											let goodsInfo = uni.getStorageSync(
												'takeout_goodsInfo'); // console.log(goodsInfo);
				...
				uni.request({
					...
					success: function(res) {
						...
						if (res.data.status == 0) {
							...
							// 	imgUrls[i].name = 'http://121.37.210.175:8001/image/' + appId + '/' + imgUrls[
							// 		i].name;
							// }
