<template>
	...
	<view class="container">
		<view
			style="height: 160rpx;background-size: 100%;display: flex;align-items: flex-end;"
        ...
        <view v-for="(i, index) in other" :key="index" class="list">
          ...
          <view class="price">￥{{i.current_price}}</view>
			...
			<view id="submitted" class="order-box">
				<view class="title">
					<view class="circle">
						<image class="font-image" src="../../static/images/two.png">
					</view>
					已提交餐品
				</view>
				<view class="menu">
					<view v-for="(i, index) in submitted.data" :key="index" class="list">
						<view class="food-name"><text>{{i.name}}</text><span class="unit">/{{i.unit}}</span></view>
						<view class="count">×{{i.count}}</view>
						<view class="price">￥{{i.discounted_price}}</view>

						<!-- style="display: flex;justify-content: space-between;margin-left: 50rpx;" -->
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			let unsubmitted = uni.getStorageSync('jtjd_order');
